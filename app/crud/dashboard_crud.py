...
def get_initial_balance_summary(company_id, bank_account_id=None):
    ...
    result = calculate_bank_balances(company_id, bank_account_id)
    # Remove the map if not needed by the caller, or just return as is.
    # The original caller expects specific keys, extra keys are fine.
    return result
...
def get_bank_account_performance(company_id, start_date=None, end_date=None, bank_account_id=None):
    try:
        ...
        # Calculate real-time balances for all accounts (independent of date filters)
        ...
        for account in all_bank_accounts:
            ...
            # Use calculated real-time balance if available, otherwise fallback to 0
            # This ensures the dashboard always shows the true current balance regardless of date filters
